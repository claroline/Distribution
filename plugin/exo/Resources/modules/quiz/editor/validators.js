...
function validateQuiz(quiz) {
  ...
  setIfError(paramErrors, 'duration', chain(parameters.duration, null, [notBlank, number, gteZero]))
  setIfError(paramErrors, 'maxAttempts', chain(parameters.maxAttempts, null, [notBlank, number, gteZero]))
  setIfError(paramErrors, 'maxAttemptsPerDay', chain(parameters.maxAttemptsPerDay, null, [notBlank, number, gteZero, (value) => {
    if (value > parameters.maxAttempts) return tex('must_be_less_than_max_attempts')
  }]))
  setIfError(paramErrors, 'maxPapers', chain(parameters.maxPapers, null, [notBlank, number, gteZero, (value) => {
  ...
  switch (picking.type) {
    case QUIZ_PICKING_TAGS:
      setIfError(pickingErrors, 'pick', chain(picking.pick, null, [notEmpty]))
      setIfError(pickingErrors, 'pageSize', chain(picking.pageSize, null, [notBlank, number, gtZero]))
      break
    case QUIZ_PICKING_DEFAULT:
    default:
      setIfError(pickingErrors, 'pick', chain(picking.pick, null, [notBlank, number, gteZero]))
      break
  }
...
function validateStep(step) {
  ...
  setIfError(
    ...
    chain(step.parameters.maxAttempts, null, [notBlank, number, gteZero])
  )
