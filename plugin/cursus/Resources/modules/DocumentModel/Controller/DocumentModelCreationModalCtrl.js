...
export default class DocumentModelCreationModalCtrl {
  constructor($http, $uibModalInstance, CourseService, title, callback) {
    this.$http = $http
  ...
  submit () {
    ...
    if (this.isValid()) {
      ...
      this.$http.post(url, {documentModelDatas: this.documentModel}).then(d => {
