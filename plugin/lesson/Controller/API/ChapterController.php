...
class ChapterController
{
     ...
     * Crete new chapter.
     *
     * @EXT\Route("/chapters/create", name="apiv2_lesson_chapter_create")
     * @EXT\Method("POST")
     * @EXT\ParamConverter("parentChapter", class="IcapLessonBundle:Chapter", options={"mapping": {"chapterSlug": "slug"}})
     *
     * @param Request $request
     * @param Lesson  $lesson
     *
     * @return JsonResponse
     */
    public function createAction(Request $request, Lesson $lesson)
    {
        return new JsonResponse(true);

        $this->checkPermission('EDIT', $lesson->getResourceNode(), [], true);

        $newChapter = $this->chapterManager->createChapter($lesson, json_decode($request->getContent(), true));
     ...
     *
     * @EXT\Route("/chapters/{chapterSlug}/edit", name="apiv2_lesson_chapter_update")
     * @EXT\Method("PUT")
     ...
     * @return JsonResponse
     */
    public function deleteAction(Lesson $lesson, Chapter $chapter)
    {
        $this->checkPermission('EDIT', $lesson->getResourceNode(), [], true);

        $this->chapterManager->deleteChapter($lesson, $chapter);
